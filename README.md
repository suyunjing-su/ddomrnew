...
- Renewal window detection by `remaining_days` or `expires_at`
- Safe fallback: when expiry fields are missing, attempt renew and rely on API error code
...
- `renewal_threshold_days`: renew when remaining days <= this value (default `7`)
- `attempt_when_unknown`: renew attempt when expiry fields are missing (default `true`)
